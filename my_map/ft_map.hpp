...
public:
    ...
    typedef ft::knots<value_type>                               knot;
...
public:
    ...
    explicit map( const Compare& compp, const Allocator& alloc = Allocator() ) : _Alloc(alloc), _comp(compp)
    {
        this->last_tmp_knot = NULL;
        this->_tree_root = NULL;
        this->_size = 0;
    ...
    map( InputIt first, InputIt last, const Compare& compp = Compare(), const Allocator& alloc = Allocator() ) : _Alloc(alloc), _comp(compp)
    {
        this->last_tmp_knot = NULL;
        this->_tree_root = NULL;
        this->_size = 0;
    ...
    T& operator[]( const Key& key )
    {
        if(!this->_tree_root)
        {
            pair<key_type, mapped_type> tmp_pair;
            tmp_pair.first = key;
            knot *tmp = insert_to_tree(tmp_pair);
            return(tmp->_pair.second);
        }
        else
        {
    ...
    {
        return (std::numeric_limits<size_t>::max() / sizeof(ft::knots<value_type>));
    ...
    iterator insert( iterator hint, const value_type& value )
    {
        ...
        if(hint)
        {
            ...
            if(new_knot)
                ...
                new_knot = find_knot(value.first, this->_tree_root);
        }
        return(new_knot);
    }
    ...
    void erase( iterator pos )
    {
        if(!this->_tree_root)
            return;
        knot *tmp = find_knot(pos->first, this->_tree_root);
        if(!tmp)
            return;
        delete_knot(*pos);
        this->_size -= 1;
    ...
    void erase( iterator first, iterator last )
    {
        ...
        for(; first != last; i++)
        {
            iterator temp = first;
            temp++;
            tmp = find_knot(first->first, this->_tree_root);
            if(!tmp)
            {
                continue;
            }
...
private:
    knot     *find_knot(Key find_key, knot* root)  const
    {
        ...
        while(1)
        {
            if(find_key == tmp->_pair.first)
                return(tmp);
            else if(find_key > tmp->_pair.first) // если то, что ищем больше, то идем в правого наследника
            {
                if(!tmp->right)
                    return(NULL);
                tmp = tmp->right;
            }
            else if(find_key < tmp->_pair.first) // если то, что ищем меньше, то идем в левого наследника
            {
                if(!tmp->left)
                    return(NULL);
                tmp = tmp->left;
    ...
    knot *add_knot_to_tree(knot *new_knot, knot *root)
    {
        ...
        if(new_knot->_pair.first == root->_pair.first)
            return(NULL);
        if(new_knot->_pair.first < root->_pair.first)
        {
            if(root->left)
    ...
    {
        if(pair_for_new_knot.first > tmp->_pair.first)
        {
        ...
        else if(pair_for_new_knot.first < tmp2->_pair.first)
        {
    ...
    knot *insert_to_tree(value_type pair_for_new_knot)
    {
        ...
        else
        {
            ...
            tmp->right = NULL;
            tmp2->left = NULL;
            if(find_knot(pair_for_new_knot.first, this->_tree_root))
            {
                tmp->right = last_tmp_knot;
                tmp2->left = first_tmp_knot;
                return NULL;
    ...
    void delete_knot(value_type data) 
    {
        ...
        {
            if (delete_knot->_pair.first == data.first)
                break;
            if (!_comp(delete_knot->_pair.first, data.first))
